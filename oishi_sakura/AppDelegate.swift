...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if ((UIScreen.main.bounds.size.height / UIScreen.main.bounds.size.width) < 1.5) {
            ...
            let bg = UIImageView(frame: CGRect.init(x: 0.0, y: 0.0, width: UIScreen.main.bounds.size.width, height: UIScreen.main.bounds.size.height))
    ...
    func applicationWillResignActive(_ application: UIApplication) {
        ...
        if (self.window?.rootViewController is GameViewController) {
            if let controller = self.window?.rootViewController as! GameViewController? {
                controller.backgroundMusicPlayer?.stop()
                print("didStopBackgroundMusicPlayer")
            }
        }
    }
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        if (self.window?.rootViewController is GameViewController) {
            if let controller = self.window?.rootViewController as! GameViewController? {
                controller.backgroundMusicPlayer?.stop()
                controller.isSoundPlaying = false
                print("applicationDidBecomActive")
            }
        }
    }
